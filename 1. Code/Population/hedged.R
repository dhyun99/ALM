...
select <- dplyr::select
options("scipen" = 100) 
#Path <-'G:/.shortcut-targets-by-id/10U-k1-n9WyT1pXVnk0sYeoQPsLxhFwT5/비즈니스랩_ALM/인구 자료 관련/2. 데이터/'
#Path <- 'C:/R/'
usepension<-function(Path, inf, wage){
  ...
  ps<-pension(showInsur_man, showInsur_woman, df_disorder_receive, df_old_receive, df_bereaved_receive) # 국민연금 수입지출
  Insurance_age <-showInsur_all_age(social_insurance , work_s, resultIncome_s)
  return(ps)
}

#적립기금추이 함수
Fund <- function(Path, inf, wage){
  #수익률 테이블 생성
  ...
  #수입지출모듈
  pension<-usepension(Path, inf, wage) # 국민연금 수입지출
...
#수입지출이 67년까지 밖에 없음. 
senario<-function(Path, inf, wage, sereturn){
  year<-c('18-20','21-30','31-40','41-50','51-60','61-70')
  return18<-4.9/100  #18-20년도 수익률
  return21<-4.8/100  #21-30년도 수익률
  return31<-4.6/100  #31-40년도 수익률
...
senario_plot<-function(Path, inf, wage, sereturn){
  sereturn<-as.numeric(sereturn)
  
  origin<-Fund(Path, inf, wage)
  senario_<-senario(Path, inf, wage, sereturn)
  ...
  my_list<-list('table'=table,  'plot'=p)
  return(my_list)
}
...
benchdata<-function(Path){
  ...
  new_data_realcost <- cbind(data.frame('수입'= diff_income,'지출'=diff_cost), log_yoy[2:nrow(log_yoy),]) 
...
#   return(mydata)
# }
simulation_con_hedged <- function(Path, ben_a, ben_b, ben_c, ben_d,income_inc, cost_inc, n){
  income_inc<-as.numeric(income_inc)
  cost_inc<-as.numeric(cost_inc)
  ben_a<-as.numeric(ben_a)
  ben_b<-as.numeric(ben_b)
...
random_simul_hedged<-function(Path, ben_a, ben_b, ben_c, ben_d,income_inc, cost_inc, n, MVO_num, RF, hedge_num, inf, wage){
  ...
  mydata<-simulation_con_hedged(Path,income_inc, cost_inc, ben_a, ben_b, ben_c, ben_d, n)
  fund_total <- Fund(Path, inf, wage)[['재정수지전망']]
...
Simul_Run<- function(Path,ben_a, ben_b, ben_c, ben_d, income_inc, cost_inc, n, MVO_num, RF, hedge_num, Risk1Name, Risk2Name, Risk1Th, Risk2Th, Risk1Yr,Risk2Yr,Risk1_Limit,Risk2_Limit, inf, wage)
{
  income_inc<-as.numeric(income_inc)
  cost_inc<-as.numeric(cost_inc)
  ben_a<-as.numeric(ben_a)
  ...
  random_simul_<-random_simul_hedged(Path, ben_a, ben_b, ben_c, ben_d,income_inc, cost_inc,n,MVO_num,RF, hedge_num, inf, wage)
  ...
  port_mean<-as.numeric(mean(portfolio_values$Return))
  port_sd<-as.numeric(mean(portfolio_values$Risk))
