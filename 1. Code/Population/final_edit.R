...
#연령별로 구분 쉽게 하기 위한 DF: df_income_m
...
social_m1<- work_m*pension_m #국민연금가입자
...
#사회보험 수입 보여주기 위한 DF: df_showInsur_m
...
for(i in 1:49){
  for(j in 1:11){
    if(j==1){
      ...
      income_earn
      k <- k+1
      pension <- social_m1[j,i] * income_earn * pension_insur #국민연금 가입자* 근로소득* 국민연금 가입률
      pension
...
social_insurance <-read_excel(paste(Path , "social_insurance_age.xlsx", sep = '')) #연령대별 사회보험 가입률
...
for(i in 1:49){
  for(j in 1:11){
    if(j==1){
      ...
      pension <- social_sub(j,i,1)* income_earn * pension_insur #국민연금 가입자* 근로소득* 국민연금 가입률
...
#연도별 사회보험 가입자수 DF: df_showInsur_sum
df_showInsur_sum <- data.frame('연도' = integer(), '국민연금'= numeric(), '건강보험'= numeric(), '고용보험'= numeric())
for(i in 1:49){
  year<- 2018+i
  pension <- colSums(df_showInsur %>% filter(연도 == year) %>%  select(국민연금))
  health <- colSums(df_showInsur %>% filter(연도 == year) %>%  select(건강보험))
  employ <- colSums(df_showInsur %>% filter(연도 == year) %>%  select(고용보험))
  df_showInsur_sum <-rbind(df_showInsur_sum, data.frame('연도' = year, '국민연금'= pension, '건강보험'= health, '고용보험'= employ))
}
rownames(df_showInsur_sum) <- c(1:49)
rowSums(df_showInsur_sum)
...
# 유족인구 추계
receive_age_m <- as.numeric(unlist(old_pensionReceive_age[7:27,3])) #2019년 노령 유족연금 수급자 수  
...
start <- 2
k <- 2
>>>>>>> 3b05e5c71c40d27bb763fb2e9e4973e9fc88901e
for(i in 1:49){ #유족인구
...
testttt_m<-bm[1:9,1,drop= FALSE]/sum_social_m[2:10,1,drop=FALSE] #2019년 장애연금 수급자수/가입자수->장애발생률?
testttt_w<-bw[1:9,1,drop= FALSE]/sum_social_w[2:10,1,drop=FALSE]


#연령별 장애연금 수급자 남녀 합DF: df_disorder_test
