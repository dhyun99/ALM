...
server <- function(input, output) {
  ...
  observeEvent(input$action, {
    ...
    usedata(paste(input$file$datapath))
  #exceldata$StockRet <-read_excel(paste(input$file$datapath, sep=""), sheet = "주식 Return")
  #exceldata$BenchRet <-read_excel(paste(input$file$datapath, sep=""), sheet = "벤치마크  print("senario make")
    Result$senario_plot <- senario_plot(getwd(), v$inf, v$wage, v$sereturn, v$category)
    print("weight make")
    Result$Simul_Plot<-Simul_Run(getwd(),v$ben_a, v$ben_b, v$ben_c, v$ben_d, v$income_inc, v$cost_inc, v$n, v$MVO_num, v$RF, v$hedge_num, v$Risk1_Name, v$Risk2_Name, v$Risk1_Th, v$Risk2_Th, v$Risk1_Yr,v$Risk2_Yr,v$Risk1_Limit,v$Risk2_Limit, v$inf, v$wage, v$category)
    print("weight Done")
    Result$Fund_result<-Fund_result(getwd(), v$inf, v$wage, v$category, Result$Simul_Plot$port_mean)
    Result$VaR<-parametric(getwd(), v$VarUse,Result$Simul_Plot$port_mean, Result$Simul_Plot$port_sd , v$VarConfidence, v$ManageTime,  v$Exposure)
    print("VaR Done")
  })
  #observeEvent(input$action, {
